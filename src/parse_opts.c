...
char *argv[];
{
	...
	if (!strcmp(argv[i], "-mc")) {
	    if (i >= (argc - 5)) {
		fprintf(stderr, "%s: not enough arguments for -mc\n", MY_NAME);
		return -1;
	    }
	    i++;
	    found = 1;
#ifdef SGI
	    MAT_COLOR[0] = atof(argv[i++]);
	    MAT_COLOR[1] = atof(argv[i++]);
	    MAT_COLOR[2] = atof(argv[i++]);
	    if ((MAT_COLOR[0] < 0.0) || (1.0 < MAT_COLOR[0])) {
		fprintf(stderr, "%s: error, illegal red color value\n", MY_NAME);
		return -1;
	    }
	    if ((MAT_COLOR[1] < 0.0) || (1.0 < MAT_COLOR[1])) {
		fprintf(stderr, "%s: error, illegal green color value\n", MY_NAME);
		return -1;
	    }
	    if ((MAT_COLOR[2] < 0.0) || (1.0 < MAT_COLOR[2])) {
		fprintf(stderr, "%s: error, illegal blue color value\n", MY_NAME);
		return -1;
	    }
#else
	    fprintf(stderr, "%s: warning, '-mc' not a valid option for non-SGI\n",
		    MY_NAME);
	    i += 3;		/* skip over material color values */
#endif
	}
	if (!strcmp(argv[i], "-k")) {
	    if (i >= (argc - 5)) {
	...
	if (!strcmp(argv[1], "-bg")) {
	    ...
	    found = 1;
#ifdef SGI
	    WIN_FOREGROUND = 0;  /* bg the job */
#else
	    fprintf(stderr, "%s: warning, '-bg' not a valid option for non-SGI\n",
		    MY_NAME);
#endif
	}
	if (!strcmp(argv[i], "-v")) {
	...
	if (!strcmp(argv[i], "-norm")) {
	    ...
	    if ((NORMAL_TYPE < 0) || (2 < NORMAL_TYPE)) {
	...
	if (!strcmp(argv[i], "-bbox")) {
	    ...
	    BBOX = 1;
#else
	    fprintf(stderr, "%s: warning, '-bbox' not a valid option for non-SGI\n",
		    MY_NAME);
#endif
	}
	if (!strcmp(argv[i], "-al")) {
	    ...
	    AUXLIGHT = 1;
#else
	    fprintf(stderr, "%s: warning, '-al' not a valid option for non-SGI\n",
		    MY_NAME);
#endif
	}
...
#ifdef SGI
#ifdef SGI_IMAGE
	    strcpy(SOUTNAME, argv[i++]);
#else
	    fprintf(stderr, "%s: warning, '-os' not a valid option - support not installed\n",
		    MY_NAME);
	    i++;		/* skip over raster name */
#endif
#else
	    fprintf(stderr, "%s: warning, '-os' not a valid option for non-SGI\n",
		    MY_NAME);
	    i++;		/* skip over raster name */
#endif
	}
...
#else
	    fprintf(stderr, "%s: vset support not installed\n", MY_NAME);
	    exit(1);
#endif
	}
	if (!strcmp(argv[i], "-byu")) {
	    if (i >= (argc - 3)) {
		fprintf(stderr, "%s: not enough arguments for -byu\n", MY_NAME);
		return -1;
	    }
	    i++;
	    found = 1;
	    strcpy(BYU_NAME, argv[i++]);
	}
#ifdef DTM
	...
	if (!strcmp(argv[i], "-raw")) {
	    if (i >= (argc - 5)) {
		fprintf(stderr, "%s: not enough arguments for -raw\n", MY_NAME);
		return -1;
	    }
	    XDIM = atoi(argv[++i]);
	    YDIM = atoi(argv[++i]);
...
usage()
{
    ...
    printf("usage: %s [-options] [<3d_sds.hdf> or <raw_file>] <threshold>\n\n",
	   MY_NAME);
    ...
    printf("           -mc <lr lg lb>               Material color\n");
    printf("           -ns <Ns>                     Specular exponent\n");
    printf("           -bg				    Background the window\n");
    printf("           -ntsc                        NTSC window\n");
    printf("           -o <hdf.r24>                 Save raster image\n");
    printf("           -os <outfile>                Save SGI image\n");
    ...
    printf("           -bbox                        Bounding box display\n");
    printf("           -al                          Auxiliary light source\n");
#endif
    printf("           -norm <n>                    Normal type\n");
    ...
    printf("The input file can be an NCSA HDF file containing a\n");
    printf("three dimensional Scientific Data Set (SDS).\n");
    printf("Additionally you can specify the -raw input option for a file\n");
    printf("of 32 bit floating point data arranged with the x dimension\n");
    printf("varying most rapidly, followed by y, and z varying most slowly\n");
